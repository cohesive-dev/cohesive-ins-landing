...
// ---- Step layout ---------------------------------------------------------
// "steps" = one question per screen with Next/Back, mirroring the FB Instant
// Form (and /restaurants, which uses the same split). The long form stays the
// control. Rather than restructure 30 field blocks into render functions, each
// Field declares the step it belongs to via `k` and hides itself when another
...
export default function CommercialPropertyForm({
  ...
  // ---- step machine (only used when layout === "steps") ------------------
  // Business questions first, contact last: the restaurant split, and it means a
  // drop-off still tells us what they own even when we get no way to reach them.
  ...
  const STEPS: StepDef[] = [
    ...
    { key: "heating", label: "The building", ok: () => !!f.heatingUpdated },
    { key: "protection", label: "Coverage", ok: () => true },
    { key: "value", label: "Coverage", ok: () => true },
    { key: "claims", label: "Coverage", ok: () => true },
  ...
  const goNext = () => {
    if (cur.ok()) setStep((n) => Math.min(n + 1, visibleSteps.length - 1));
  };
  ...
  // Per-screen pixel event on REACH (Back doesn't re-fire, since track() is
  // once-per-session per name). Events Manager then shows the drop-off curve
  // between screens the same way the restaurant funnel does.
  const [furthestStep, setFurthestStep] = useState(0);
  useEffect(() => {
    if (!stepped || !cur) return;
    track(`CPStep_${cur.key}`);
  ...
  // Stay on the current step key even when disqualified: dropping to null falls
  ...
  return (
    ...
    <main className="min-h-screen bg-white">
      ...
      >
        {stepped && !disqualified && (
          <div className="space-y-2">
            <div className="flex items-baseline justify-between">
              <span className="text-sm font-semibold text-[#27455C]">
                {cur.label}
              </span>
              {isLast && (
                <span className="text-xs font-medium text-[#2040E7]">
                  Last one
                </span>
              )}
            </div>
            <div className="h-1.5 w-full overflow-hidden rounded-full bg-[#EEF1FF]">
              <div
                className="h-full rounded-full bg-[#2040E7] transition-all"
                style={{
            ...
            {step === 0 && (
              <p className="text-xs text-[#6B6D71]">Takes about a minute.</p>
            )}
            {cur.hint && (
              <p className="text-xs text-[#6B6D71]">{cur.hint}</p>
            )}
          </div>
        )}
        {/* Contact */}
        <Section title="Your contact info">
          <Field k="contact" label="Full name" required>
        ...
        {qualified && (
          <Section title="Your building">
            ...
            <div className="space-y-4 rounded-xl border border-[#EEF1FF] bg-[#FBFCFF] p-4">
              ...
              <Field k="systems" label="When was the electrical last updated?" required>
              ...
              {UPDATED_RANGES.has(f.electricalUpdated ?? "") && (
                <Field k="systems" label="Electrical update year" hint="Best guess is fine.">
              ...
              {UPDATED_RANGES.has(f.plumbingUpdated ?? "") && (
                <Field k="systems" label="Plumbing update year" hint="Best guess is fine.">
        ...
        {qualified && (
          <Section title="Claims history">
            <Field k="claims" label="Any property or liability claims in the last 5 years?">
          ...
          <div className="space-y-3">
            ...
            {step > 0 && (
              <button
                type="button"
                onClick={goBack}
                className="w-full text-center text-sm font-medium text-[#6B6D71] underline underline-offset-4"
              >
                Back
              </button>
            )}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  // In "steps" the screen already carries a section label above the progress
  // bar, so the per-section heading (and its spacing) would double up.
  const stepped = useContext(StepCtx) !== null;
  if (stepped) return <section className="space-y-4">{children}</section>;
  return (
...
function Field({
  ...
  hint?: string;
  required?: boolean;
  // Step this field belongs to. In "steps" layout only the active step renders;
  // in "long" layout StepCtx is null and every field renders as before.
  k?: string;
