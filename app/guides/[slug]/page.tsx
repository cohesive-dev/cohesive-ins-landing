import StartupBundle from "@/components/guides/StartupBundle";
...
export default async function GuidePage({ params }: Props) {
  const guide = getStartupGuide((await params).slug);
  if (!guide) notFound();
  const bundle = !guide.noQuote && guide.sections.some((s) => s.id === "cohesive-ai-referrals");
  ...
  return <main className="guide-article">
    ...
    <div className="mx-auto max-w-6xl px-5 py-10 sm:py-14">
      ...
      <div className="mt-10 grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_260px]">
        ...
        <aside className="rounded-2xl border border-blue-100 bg-[#F7F9FF] p-6 lg:sticky lg:top-8 print:hidden">
          {guide.noQuote ? <><h2 className="text-xl font-bold">Plan your next step</h2><p className="mt-3 text-sm leading-6 text-slate-600">{guide.noQuoteReason}</p><a href="#startup-checklist" className="mt-4 block font-semibold text-blue-700">Use the free startup checklist →</a></> : <><p className="text-xs font-bold uppercase tracking-wide text-blue-700">Cohesive Insurance</p><h2 className="mt-3 text-xl font-bold">{bundle ? "Insurance and help finding customers." : "Get insured before your first job."}</h2><p className="mb-5 mt-3 text-sm leading-6 text-slate-600">{bundle ? (guide.kind === "growth" ? GROWTH_BUNDLE.summary : STARTUP_BUNDLE.summary) : "Start with Cohesive for business insurance. Share your operations, state, start date, and any landlord or customer insurance requirements."}</p><GuideQuoteLink slug={guide.slug} label={bundle ? (guide.kind === "growth" ? GROWTH_BUNDLE.cta : STARTUP_BUNDLE.cta) : undefined} /><Link href={guide.insurancePath} className="mt-4 block text-sm font-semibold text-blue-700 hover:underline">Read about {guide.tradeLabel?.toLowerCase()} insurance →</Link></>}
        </aside>
      </div>
    </div>
    {getRelatedGuides(guide).length > 0 && <section aria-labelledby="related-guides" className="border-t border-slate-200 print:hidden"><div className="mx-auto max-w-6xl px-5 py-10"><h2 id="related-guides" className="text-2xl font-bold">{guide.kind === "growth" ? "Explore another local market" : "Keep planning your business"}</h2><div className="mt-5 grid gap-4 sm:grid-cols-2">{getRelatedGuides(guide).map((related) => <Link key={related.slug} href={`/guides/${related.slug}`} className="rounded-xl border border-slate-200 p-4 text-sm font-semibold leading-6 text-blue-700 hover:border-blue-500">{related.title} →</Link>)}</div></div></section>}
    ...
    {!guide.noQuote && <section id="quote" aria-labelledby="guide-quote-heading" className="scroll-mt-5 border-t border-slate-200 bg-slate-50 px-5 py-12 print:hidden"><div className="mx-auto mb-6 max-w-2xl text-center"><h2 id="guide-quote-heading" className="text-3xl font-bold">Get business insurance with Cohesive</h2><p className="mt-3 leading-7 text-slate-600">Tell Cohesive about your business and the coverage you need to get started. Include any GC, property manager, landlord, or venue requirements in your description.</p></div><StartupQuoteForm bundleOffer={bundle} slug={guide.slug} industry={guide.industry ?? "Restaurants"} tradeLabel={guide.tradeLabel ?? "Restaurant"} initialState={guide.stateSlug} states={STARTUP_STATES.map(({slug, name}) => ({slug, name}))} /></section>}
  </main>;
}
